...
def pegar_ultimo_resultado(driver):
    try:
        ...
        # Captura hora e minuto diretamente dos atributos
        hora_attr = cell.get_attribute("data-hour")
        minuto_attr = cell.get_attribute("data-minute")

        # =================== NOVA VERIFICAÇÃO ===================
        # Ignorar resultados inválidos do site (virada de dia)
        if hora_attr == "99" and minuto_attr == "99":
            return None

        hora_text = cell.find_element(By.CSS_SELECTOR, ".cell__date").text.strip()
...
def monitorar_site():
    ...
    while True:
        try:
            ...
            # =================== NOVA VERIFICAÇÃO ===================
            # Pula resultados inválidos ou já processados
